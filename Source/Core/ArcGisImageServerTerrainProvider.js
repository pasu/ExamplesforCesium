...
define([
  ...
  return LercCodec;
    ...
    ArcGisImageServerTerrainProvider.prototype.requestTileGeometry = function(x, y, level) {
        var url = "https://elevation3d.arcgis.com/arcgis/rest/services/WorldElevation3D/Terrain3D/ImageServer/tile/{z}/{y}/{x}";
        url = url.replace('{z}', level).replace('{x}', x).replace('{y}', y);

        var promise;

        function tileLoader(tileUrl) {
            return loadArrayBuffer(tileUrl);
        }
